...
export const formatPhone = (phone: string, config: FormatPhoneConfig): string => {
  const shouldForceDialCode = config.disableDialCodeAndPrefix ? false : config.forceDialCode
  ...
  if (!phoneValue) {
    if ((shouldInsertDialCodeOnEmpty && !phoneValue.length) || shouldForceDialCode) {
      return handleResult(`${config.prefix}${config.dial_code}${config.charAfterDialCode}`)
    }
  ...
  if (config.dial_code.startsWith(phoneValue) && !config.disableDialCodeAndPrefix) {
  ...
  if (!phoneValue.startsWith(config.dial_code) && !config.disableDialCodeAndPrefix) {
    if (shouldForceDialCode) {
      return handleResult(`${config.prefix}${config.dial_code}${config.charAfterDialCode}`)
    }
  ...
  phoneRightSide = applyMask({
    ...
    trimNonMaskCharsLeftover:
      config.trimNonDigitsEnd || (config.disableDialCodeAndPrefix && phoneRightSide.length === 0),
  })
