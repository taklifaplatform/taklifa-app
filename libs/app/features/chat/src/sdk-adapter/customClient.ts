...
import { Database } from '@zix/supabase/types'
...
export class CustomClient {
  ...
  _user?: OwnUserResponse<ExtendableGenerics> | UserResponse<ExtendableGenerics>
  activeChannels: {
    ...
    const channelsData = (data || []).map((channel) => {
      return {
        ...
        messages: (channel.messages || []).map((message) => ({
          ...message,
          reply_count: message.reply_count?.length ? message.reply_count[0].count : 0,
  ...
  _handleClientEvent(event: Event<StreamChatGenerics>) {
    ...
    if (
      (event.type === 'channel.deleted' || event.type === 'notification.channel_deleted') &&
      event.cid
    ) {
  ...
  channel(
    ...
    custom: ChannelData<ExtendableGenerics> = {} as ChannelData<ExtendableGenerics>
  ) {
    ...
    if (!channelIDOrCustom) {
      return new Channel<ExtendableGenerics>(this, channelType, undefined, custom)
    }
   ...
   * @return {channel} The channel object, initialize it using channel.watch()
   */
  getChannelByMembers = (channelType: string, custom: ChannelData<ExtendableGenerics>) => {
    ...
    const channel = new Channel<ExtendableGenerics>(this, channelType, undefined, custom)
  ...
  getChannelById = (
    ...
    // )
    // console.log('===========')
    const channel = new Channel<ExtendableGenerics>(this, channelType, channelID, custom)
    ...
    const routes = [
      ...
      },
      {
        ...
        handler: async function (channelId) {
          console.log('/////////////////////')
          console.log('customClient->post::', JSON.stringify(data.message, null, 2))
          console.log('===========')
          ...
          console.log('===========')
          console.log('SUPACustomClient->post:chat_messages::', JSON.stringify({ result }, null, 2))
          console.log('===========')
          if (result.error) {
            return {
              error: result.error,
  ...
  async deleteMessage(messageID: string, hardDelete?: boolean) {
    ...
    return this.supabase.schema('chat').from('messages').delete().match({ id: messageID })
  }
