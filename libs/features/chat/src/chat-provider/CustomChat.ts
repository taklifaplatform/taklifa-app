...
import { AuthenticatedUserTransformer, OpenAPI } from "@zix/api";
...
> {
  ...
  */
  ...
  async getDevices(userID?: string) {
    ...
    >(this.baseURL + "/devices", userID ? { user_id: userID } : {});
  }
  ...
  async reactivateUser(userID: string, options?: ReactivateUserOptions) {
    return await this.post<
      APIResponse & { user: UserResponse<StreamChatGenerics> }
    >(this.baseURL + `/users/${userID}/reactivate`, { ...options });
  }
  ...
  async deactivateUser(userID: string, options?: DeactivateUsersOptions) {
    return await this.post<
      APIResponse & { user: UserResponse<StreamChatGenerics> }
    >(this.baseURL + `/users/${userID}/deactivate`, { ...options });
  }
  ...
  getUserAgent() {
    return (
      this.userAgent || `zix-chat-javascript-client-${process.env.PKG_VERSION}`
    );
  }
