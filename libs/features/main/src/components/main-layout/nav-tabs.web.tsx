...
export const NavTabs = (props: TabsProps) => {
  ...
  const currentTab = nextRouter.pathname?.startsWith('/account') ? '/account' : nextRouter.pathname
  ...
  return (
    <Tabs
      $gtSm={{ marginBottom: '$-0.75', }} // for the active TabsRovingIndicator to look good
      value={currentTab}
    ...
    >
      ...
      >
        ...
        <Tab value="/apps/chat/app" onInteraction={handleOnInteraction}>
          Chat Bots
        </Tab>
        {/* <Tab value="/account" onInteraction={handleOnInteraction}>
          Settings
        </Tab> */}
      </Tabs.List>
    </Tabs>
...
const Tab = (props: TabsTabProps) => (
  <Tabs.Tab unstyled justifyContent="flex-end" margin="$1.5" {...props}>
