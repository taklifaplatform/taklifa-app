...
import { isErrorResponse } from "stream-chat/src/errors";
import {
  addFileToFormData,
  chatCodes,
...
> {
  private static _instance?: unknown | ZixChat; // type is undefined|ZixChat, unknown is due to TS limitations with statics
  supabase: SupabaseClient<Database>;
  _user?: OwnUserResponse<ZixChatGenerics> | UserResponse<ZixChatGenerics>;
  activeChannels: {
    [key: string]: Channel<ZixChatGenerics>;
  };
  anonymous: boolean;
  persistUserOnConnectionFailure?: boolean;
  zixChatClientInstance: ZixChatApiClient;
  baseURL?: string = "";
  browser: boolean;
  cleaningIntervalRef?: NodeJS.Timeout;
  clientID?: string;
  configs: Configs<ZixChatGenerics>;
  supabaseChannels: Record<string, any>;
  key: string;
  listeners: Record<string, Array<(event: Event<ZixChatGenerics>) => void>>;
  logger: Logger;
  /**
  ...
  mutedChannels: ChannelMute<ZixChatGenerics>[];
  mutedUsers: Mute<ZixChatGenerics>[];
  state: ClientState<ZixChatGenerics>;
  user?: OwnUserResponse<ZixChatGenerics> | UserResponse<ZixChatGenerics>;
  options: ZixChatOptions;
  userAgent?: string;
  userID?: string;
  wsConnection: ZixStableWSConnection<ZixChatGenerics> | null;
  wsPromise: ConnectAPIResponse<ZixChatGenerics> | null;
   ...
   * Initialize a client
   *
   * **Only use constructor for advanced usages. It is strongly advised to use `ZixChat.getInstance()` instead of `new ZixChat()` to reduce integration issues due to multiple WebSocket connections**
   * @param {string} key - the api key
   * @param {string} [secret] - the api secret
   * @param {ZixChatOptions} [options] - additional options, here you can pass custom options to axios instance
   ...
   * new ZixChat('api_key', { warmUp:true, timeout:5000 })
  ...
  constructor() {
    ...
    // replace with supabaseInstance client
    this.zixChatClientInstance = new ZixChatApiClient(this);
    ...
    this.persistUserOnConnectionFailure = true;
    ...
    // this.zixChatClientInstance.defaults.paramsSerializer = axiosParamsSerializer
     ...
     *
     * e.g.,
     * const client = new ZixChat('api_key', {}, {
  ...
  >(): ZixChat<ZixChatGenerics> {
    ...
    return ZixChat._instance as ZixChat<ZixChatGenerics>;
  }
   ...
   *
   * @param {OwnUserResponse<ZixChatGenerics> | UserResponse<ZixChatGenerics>} user Data about this user. IE {name: "john"}
   *
   * @return {ConnectAPIResponse<ZixChatGenerics>} Returns a promise that resolves when the connection is setup
   */
  // TODO: change user type
  connectUser = async (user: any) => {
    if (!user.id) {
    ...
    // console.log('###### wsPromise::', wsPromise)
  };
  ...
  async revokeUsersToken(userIDs: string[], before?: Date | string | null) {
    ...
    const users: PartialUserUpdate<ZixChatGenerics>[] = [];
    for (const userID of userIDs) {
      users.push({
        id: userID,
        set: <Partial<UserResponse<ZixChatGenerics>>> {
  ...
  async getAppSettings() {
    return await this.get<AppSettingsAPIResponse<ZixChatGenerics>>(
      this.baseURL + "/app",
    );
  }
  ...
  */
   ...
   *
   * @param {UserResponse<ZixChatGenerics>} user Data about this user. IE {name: "john"}
   *
   * @return {ConnectAPIResponse<ZixChatGenerics>} Returns a promise that resolves when the connection is setup
   */
  async setGuestUser(user: UserResponse<ZixChatGenerics>) {
    let response:
      | { access_token: string; user: UserResponse<ZixChatGenerics> }
      | undefined;
    this.anonymous = true;
    try {
      response = await this.post<
        APIResponse & {
          access_token: string;
          user: UserResponse<ZixChatGenerics>;
        }
    ...
    const { created_at, updated_at, last_active, online, ...guestUser } =
      response.user;
    return await this.connectUser(
      guestUser as UserResponse<ZixChatGenerics>,
   ...
   *
   * @param {EventHandler<ZixChatGenerics> | string} callbackOrString  The event type to listen for (optional)
   * @param {EventHandler<ZixChatGenerics>} [callbackOrNothing] The callback to call
   *
   * @return {{ unsubscribe: () => void }} Description
   */
  on(callback: EventHandler<ZixChatGenerics>): { unsubscribe: () => void };
  on(
    eventType: string,
    callback: EventHandler<ZixChatGenerics>,
  ): { unsubscribe: () => void };
  on(
    callbackOrString: EventHandler<ZixChatGenerics> | string,
    callbackOrNothing?: EventHandler<ZixChatGenerics>,
  ): { unsubscribe: () => void } {
    const key = callbackOrNothing ? (callbackOrString as string) : "all";
    const callback = callbackOrNothing
      ? callbackOrNothing
      : (callbackOrString as EventHandler<ZixChatGenerics>);
    ...
    return {
      unsubscribe: () => {
        ...
        this.listeners[key] = this.listeners[key].filter((el) =>
          el !== callback
        );
      },
    };
   ...
   */
  off(callback: EventHandler<ZixChatGenerics>): void;
  off(eventType: string, callback: EventHandler<ZixChatGenerics>): void;
  off(
    callbackOrString: EventHandler<ZixChatGenerics> | string,
    callbackOrNothing?: EventHandler<ZixChatGenerics>,
  ) {
    ...
    this.listeners[key] = this.listeners[key].filter((value) =>
      value !== callback
    );
  }
  ...
  doAxiosRequest = async <T>(
    ...
    try {
      ...
      switch (type) {
        case "get":
          response = await this.zixChatClientInstance.get(url, requestConfig);
          break;
        case "delete":
          response = await this.zixChatClientInstance.delete(
            url,
            requestConfig,
          );
          break;
        case "post":
          response = await this.zixChatClientInstance.post(
            url,
            data,
            requestConfig,
          );
          break;
        case "postForm":
          response = await this.zixChatClientInstance.postForm(
            url,
            data,
            requestConfig,
          );
          break;
        case "put":
          response = await this.zixChatClientInstance.put(
            url,
            data,
            requestConfig,
          );
          break;
        case "patch":
          response = await this.zixChatClientInstance.patch(
            url,
            data,
            requestConfig,
          );
          break;
        case "options":
          response = await this.zixChatClientInstance.options(
            url,
            requestConfig,
          );
          break;
      ...
      console.log("=======");
      console.log("doRequest::error", e, { url, type });
      console.log("=======");
  ...
  sendFile(
    ...
    const formData = addFileToFormData(
      uri,
      name,
  ...
  errorFromResponse(
    ...
    err = new ErrorFromResponse(`ZixChat error HTTP code: ${response.status}`);
    if (response.data && response.data.code) {
      err = new Error(
        `ZixChat error code ${response.data.code}: ${response.data.message}`,
      );
  ...
  dispatchEvent = (event: Event<ZixChatGenerics>) => {
  ...
  handleEvent = (messageEvent) => {
    ...
    const event = JSON.parse(jsonString) as Event<ZixChatGenerics>;
   ...
   *
   * @param {UserResponse<ZixChatGenerics>} user
   */
  _updateMemberWatcherReferences = (user: UserResponse<ZixChatGenerics>) => {
   ...
   * @param {UserResponse<ZixChatGenerics>} user
   */
  _updateUserMessageReferences = (user: UserResponse<ZixChatGenerics>) => {
    const refMap = this.state.userChannelReferences[user.id] || {};
   ...
   * @param {Event} event
   */
  _handleUserEvent = (event: Event<ZixChatGenerics>) => {
    if (!event.user) {
      return;
    }
  ...
  _handleClientEvent(event: Event<ZixChatGenerics>) {
    const client = this;
  ...
  _callClientListeners = (event: Event<ZixChatGenerics>) => {
    const client = this;
    // gather and call the listeners
    const listeners: Array<(event: Event<ZixChatGenerics>) => void> = [];
  ...
  recoverState = async () => {
    ...
    if (cids.length && this.recoverStateOnReconnect) {
      ...
      await this.queryChannels(
        { cid: { $in: cids } } as ChannelFilters<ZixChatGenerics>,
        { last_message_at: -1 },
        { limit: 30 },
      );
      ...
      // cids.forEach((cid) => {
      //   console.log('=================')
      //   console.log('subscribe::', cid)
      //   console.log('=================')
      //   this.supabaseChannels[cid] = this.supabase.channel(cid)

      //   this.supabaseChannels[cid].subscribe(async (status) => {
      //     if (status !== 'SUBSCRIBED') {
      //       return
      //     }
      //     this.supabaseChannels[cid].on('*', (payload) => {
      //       console.log('@@@@@@@@@@@@@@@@@@@')
      //       console.log('new event::', cid, payload)
      //       console.log('@@@@@@@@@@@@@@@@@@@')
      //     })
      //   })
      // })
      ...
      this.logger(
        "info",
        "client:recoverState() - Querying channels finished",
        {
          tags: ["connection", "client"],
        },
      );
  ...
  async connect() {
    ...
    //   this._sayHi()
    // }
    // The ZixStableWSConnection handles all the reconnection logic.
    this.wsConnection = new ZixStableWSConnection<ZixChatGenerics>({
      client: this,
    });
    // if (this.options.wsConnection && this.node) {
    //   // Intentionally avoiding adding ts generics on wsConnection in options since its only useful for unit test purpose.
    //   ;(this.options.wsConnection as unknown as ZixStableWSConnection<ZixChatGenerics>).setClient(this)
    //   this.wsConnection = this.options
    //     .wsConnection as unknown as ZixStableWSConnection<ZixChatGenerics>
    // } else {
    //   this.wsConnection = new ZixStableWSConnection<ZixChatGenerics>({
    //     client: this,
    //   })
    ...
    try {
      ...
      console.log("=================");
      console.log("this.wsConnection.connect::error", err);
      console.log("=================");
      ...
      //   this.wsFallback = new WSConnectionFallback<ZixChatGenerics>({
      //     client: this,
      //   })
      //   return await this.wsFallback.connect()
      // }
  ...
  _sayHi() {
    ...
    this.doAxiosRequest("get", this.baseURL + "/hi", null, opts).catch((e) => {
      console.log("postInsights::http_hi_failed", {
        api_key: this.key,
        err: e,
        client_request_id,
      });
   ...
   *
   * @param {UserFilters<ZixChatGenerics>} filterConditions MongoDB style filter conditions
   * @param {UserSort<ZixChatGenerics>} sort Sort options, for instance [{last_active: -1}].
   * When using multiple fields, make sure you use array of objects to guarantee field order, for instance [{last_active: -1}, {created_at: 1}]
   * @param {UserOptions} options Option object, {presence: true}
   *
   * @return {Promise<{ users: Array<UserResponse<ZixChatGenerics>> }>} User Query Response
   */
  async queryUsers(
    filterConditions: UserFilters<ZixChatGenerics>,
    ...
    const data = await this.get<
      APIResponse & { users: Array<UserResponse<ZixChatGenerics>> }
    >(
      this.baseURL + "/users",
      {
        payload: {
   ...
   *
   * @return {Promise<BannedUsersResponse<ZixChatGenerics>>} Ban Query Response
   */
  async queryBannedUsers(
    ...
    return await this.get<BannedUsersResponse<ZixChatGenerics>>(
   ...
   *
   * @return {Promise<MessageFlagsResponse<ZixChatGenerics>>} Message Flags Response
   */
  async queryMessageFlags(
    ...
    return await this.get<MessageFlagsResponse<ZixChatGenerics>>(
   ...
   *
   * @param {ChannelFilters<ZixChatGenerics>} filterConditions object MongoDB style filters
   * @param {ChannelSort<ZixChatGenerics>} [sort] Sort options, for instance {created_at: -1}.
   * When using multiple fields, make sure you use array of objects to guarantee field order, for instance [{last_updated: -1}, {created_at: 1}]
   * @param {ChannelOptions} [options] Options object
   * @param {ChannelStateOptions} [stateOptions] State options object. These options will only be used for state management and won't be sent in the request.
   * - stateOptions.skipInitialization - Skips the initialization of the state for the channels matching the ids in the list.
   *
   * @return {Promise<{ channels: Array<ChannelAPIResponse<AZixChatGenerics>>}> } search channels response
   */
  async queryChannels(
    filterConditions: ChannelFilters<ZixChatGenerics>,
    sort: ChannelSort<ZixChatGenerics> = [],
    ...
    const data = await this.post<QueryChannelsAPIResponse<ZixChatGenerics>>(
  ...
  hydrateActiveChannels(
    channelsFromApi: ChannelAPIResponse<ZixChatGenerics>[] = [],
    stateOptions: ChannelStateOptions = {},
  ) {
    ...
    const channels: Channel<ZixChatGenerics>[] = [];
   ...
   *
   * @param {ChannelFilters<ZixChatGenerics>} filterConditions MongoDB style filter conditions
   * @param {MessageFilters<ZixChatGenerics> | string} query search query or object MongoDB style filters
   * @param {SearchOptions<ZixChatGenerics>} [options] Option object, {user_id: 'tommaso'}
   *
   * @return {Promise<SearchAPIResponse<ZixChatGenerics>>} search messages response
   */
  async search(
    filterConditions: ChannelFilters<ZixChatGenerics>,
    query: string | MessageFilters<ZixChatGenerics>,
    options: SearchOptions<ZixChatGenerics> = {},
  ) {
    if (options.offset && options.next) {
      throw Error(`Cannot specify offset with next`);
    }
    const payload: SearchPayload<ZixChatGenerics> = {
      filter_conditions: filterConditions,
      ...options,
      sort: options.sort
        ? normalizeQuerySort<SearchMessageSortBase<ZixChatGenerics>>(
          options.sort,
        )
        : undefined,
    };
    ...
    return await this.get<SearchAPIResponse<ZixChatGenerics>>(
      this.baseURL + "/search",
      { payload },
    );
  }
  ...
  setLocalDevice(device: BaseDeviceFields) {
    if (
      ...
      throw new Error(
        "you can only set device before opening a websocket connection",
      );
    }
   ...
   *
   * @return {Device<ZixChatGenerics>[]} Array of devices
   */
  async getDevices(userID?: string) {
    return await this.get<
      APIResponse & { devices?: Device<ZixChatGenerics>[] }
    >(
  ...
  _addChannelConfig({ cid, config }: ChannelResponse<ZixChatGenerics>) {
   ...
   * @param {string | ChannelData<ZixChatGenerics> | null} [channelIDOrCustom]   The channel ID, you can leave this out if you want to create a conversation channel
  ...
  channel(
    ...
    custom?: ChannelData<ZixChatGenerics>,
  ): Channel<ZixChatGenerics>;
  channel(
    channelType: string,
    custom?: ChannelData<ZixChatGenerics>,
  ): Channel<ZixChatGenerics>;
  channel(
    channelType: string,
    channelIDOrCustom?: string | ChannelData<ZixChatGenerics> | null,
    custom: ChannelData<ZixChatGenerics> = {} as ChannelData<ZixChatGenerics>,
  ) {
    if (!this.userID) {
      throw Error(
    ...
    if (!channelIDOrCustom) {
      return new Channel<ZixChatGenerics>(this, channelType, undefined, custom);
    }
  ...
  getChannelByMembers = (
    ...
    const channel = new Channel<ZixChatGenerics>(
      this,
      channelType,
      undefined,
      custom,
   ...
   *
   * @param {PartialUserUpdate<ZixChatGenerics>} partialUserObject which should contain id and any of "set" or "unset" params;
   * example: {id: "user1", set:{field: value}, unset:["field2"]}
   *
   * @return {Promise<{ users: { [key: string]: UserResponse<ZixChatGenerics> } }>} list of updated users
   */
  async partialUpdateUser(
    partialUserObject: PartialUserUpdate<ZixChatGenerics>,
  ) {
   ...
   *
   * @param {UserResponse<ZixChatGenerics>[]} users list of users
   *
   * @return {Promise<{ users: { [key: string]: UserResponse<ZixChatGenerics> } }>}
   */
  async upsertUsers(users: UserResponse<ZixChatGenerics>[]) {
    const userMap: { [key: string]: UserResponse<ZixChatGenerics> } = {};
    ...
    return await this.post<
      APIResponse & {
        users: { [key: string]: UserResponse<ZixChatGenerics> };
      }
   ...
   *
   * @param {UserResponse<ZixChatGenerics>} userObject user object, the only required field is the user id. IE {id: "myuser"} is valid
   *
   * @return {Promise<{ users: { [key: string]: UserResponse<ZixChatGenerics> } }>}
   */
  upsertUser(userObject: UserResponse<ZixChatGenerics>) {
   ...
   *
   * @param {PartialUserUpdate<ZixChatGenerics>[]} users list of partial update requests
   *
   * @return {Promise<{ users: { [key: string]: UserResponse<ZixChatGenerics> } }>}
   */
  async partialUpdateUsers(users: PartialUserUpdate<ZixChatGenerics>[]) {
    for (const userObject of users) {
      if (!userObject.id) {
        throw Error("User ID is required when updating a user");
      }
  ...
  async deleteUser(
    ...
    return await this.delete<
      APIResponse & { user: UserResponse<ZixChatGenerics> } & {
        task_id?: string;
      }
  ...
  async exportUser(userID: string, options?: Record<string, string>) {
    return await this.get<
      APIResponse & {
        messages: MessageResponse<ZixChatGenerics>[];
        reactions: ReactionResponse<ZixChatGenerics>[];
        user: UserResponse<ZixChatGenerics>;
      }
   ...
   * @param {BanUserOptions<ZixChatGenerics>} [options]
   * @returns {Promise<APIResponse>}
   */
  async banUser(
    targetUserID: string,
    options?: BanUserOptions<ZixChatGenerics>,
  ) {
   ...
   * @param {MuteUserOptions<ZixChatGenerics>} [options]
   * @returns {Promise<MuteUserResponse<ZixChatGenerics>>}
   */
  async muteUser(
    targetID: string,
    userID?: string,
    options: MuteUserOptions<ZixChatGenerics> = {},
  ) {
    return await this.post<MuteUserResponse<ZixChatGenerics>>(
  ...
  async flagMessage(
    ...
    return await this.post<FlagMessageResponse<ZixChatGenerics>>(
  ...
  async flagUser(targetID: string, options: { user_id?: string } = {}) {
    return await this.post<FlagUserResponse<ZixChatGenerics>>(
      this.baseURL + "/moderation/flag",
      {
   ...
   *
   * @return {Promise<FlagsResponse<ZixChatGenerics>>} Flags Response
   */
  async _queryFlags(
    ...
    return await this.post<FlagsResponse<ZixChatGenerics>>(
   ...
   *
   * @return {Promise<FlagReportsResponse<ZixChatGenerics>>} Flag Reports Response
   */
  async _queryFlagReports(
    ...
    // Return a list of message flags
    return await this.post<FlagReportsResponse<ZixChatGenerics>>(
  ...
  async _reviewFlagReport(
    ...
    return await this.patch<ReviewFlagReportResponse<ZixChatGenerics>>(
      this.baseURL + `/moderation/reports/${id}`,
      {
   ...
   * @param {MarkAllReadOptions<ZixChatGenerics>} [data]
   *
   ...
   *
   * @param {MarkChannelsReadOptions <ZixChatGenerics>} [data]
   *
   * @return {Promise<APIResponse>}
   */
  async markChannelsRead(data: MarkChannelsReadOptions<ZixChatGenerics> = {}) {
    await this.post<APIResponse>(this.baseURL + "/channels/read", { ...data });
  }

  createCommand(data: CreateCommandOptions<ZixChatGenerics>) {
    return this.post<CreateCommandResponse<ZixChatGenerics>>(
  ...
  getCommand(name: string) {
    return this.get<GetCommandResponse<ZixChatGenerics>>(
      this.baseURL + `/commands/${name}`,
    );
  }

  updateCommand(name: string, data: UpdateCommandOptions<ZixChatGenerics>) {
    return this.put<UpdateCommandResponse<ZixChatGenerics>>(
      this.baseURL + `/commands/${name}`,
      data,
    );
  }
  ...
  deleteCommand(name: string) {
    return this.delete<DeleteCommandResponse<ZixChatGenerics>>(
      this.baseURL + `/commands/${name}`,
    );
  }

  listCommands() {
    return this.get<ListCommandsResponse<ZixChatGenerics>>(
      this.baseURL + `/commands`,
    );
  }

  createChannelType(data: CreateChannelOptions<ZixChatGenerics>) {
    const channelData = Object.assign({}, { commands: ["all"] }, data);
    return this.post<CreateChannelResponse<ZixChatGenerics>>(
  ...
  getChannelType(channelType: string) {
    return this.get<GetChannelTypeResponse<ZixChatGenerics>>(
  ...
  updateChannelType(
    channelType: string,
    data: UpdateChannelOptions<ZixChatGenerics>,
  ) {
    return this.put<UpdateChannelResponse<ZixChatGenerics>>(
      this.baseURL + `/channeltypes/${channelType}`,
      data,
    );
  }
  ...
  listChannelTypes() {
    return this.get<ListChannelResponse<ZixChatGenerics>>(
      this.baseURL + `/channeltypes`,
    );
  }
   ...
   *
   * @return {MessageResponse<ZixChatGenerics>} Response that includes the message
   */
  async translateMessage(messageId: string, language: string) {
    return await this.post<APIResponse & MessageResponse<ZixChatGenerics>>(
  ...
  pinMessage(
    ...
    return this.partialUpdateMessage(
      ...
      } as unknown as PartialMessageUpdate<ZixChatGenerics>,
      pinnedBy,
    );
  }
   ...
   *
   * @param {Omit<MessageResponse<ZixChatGenerics>, 'mentioned_users'> & { mentioned_users?: string[] }} message object, id needs to be specified
   * @param {string | { id: string }} [userId]
   * @param {boolean} [options.skip_enrich_url] Do not try to enrich the URLs within message
   *
   * @return {{ message: MessageResponse<ZixChatGenerics> }} Response that includes the message
   */
  async updateMessage(
    message: UpdatedMessage<ZixChatGenerics>,
    userId?: string | { id: string },
    ...
    if (userId != null) {
      if (isString(userId)) {
        ...
        clonedMessage.user = {
          id: userId.id,
        } as UserResponse<ZixChatGenerics>;
      }
    }
    ...
    return await this.post<UpdateMessageAPIResponse<ZixChatGenerics>>(
   ...
   *
   * @param {PartialUpdateMessage<ZixChatGenerics>}  partialMessageObject which should contain id and any of "set" or "unset" params;
   *         example: {id: "user1", set:{text: "hi"}, unset:["color"]}
   * @param {string | { id: string }} [userId]
   *
   * @param {boolean} [options.skip_enrich_url] Do not try to enrich the URLs within message
   *
   * @return {{ message: MessageResponse<ZixChatGenerics> }} Response that includes the updated message
   */
  async partialUpdateMessage(
    id: string,
    partialMessageObject: PartialMessageUpdate<ZixChatGenerics>,
    userId?: string | { id: string },
    options?: UpdateMessageOptions,
  ) {
    if (!id) {
      throw Error(
        "Please specify the message id when calling partialUpdateMessage",
      );
    }
    let user = userId;
    ...
    return await this.put<UpdateMessageAPIResponse<ZixChatGenerics>>(
  ...
  async deleteMessage(messageID: string, hardDelete?: boolean) {
    ...
    return await this.delete<
      APIResponse & { message: MessageResponse<ZixChatGenerics> }
    >(
  ...
  async getMessage(messageID: string) {
    return await this.get<GetMessageAPIResponse<ZixChatGenerics>>(
  ...
  updatePermission(
    id: string,
    permissionData: Omit<CustomPermissionOptions, "id">,
  ) {
  ...
  async sendUserCustomEvent(targetUserID: string, event: UserCustomEvent) {
    return await this.post<APIResponse>(
      `${this.baseURL}/users/${targetUserID}/event`,
      {
        event,
      },
    );
  ...
  async queryCampaigns(
    ...
    return await this.get<{
      ...
      channels?: Record<string, ChannelResponse<ZixChatGenerics>>;
      users?: Record<string, UserResponse<ZixChatGenerics>>;
    }>(this.baseURL + `/campaigns`, {
      payload: {
        filter_conditions: filters,
        ...options,
  ...
  async testCampaign(id: string, params: { users: string[] }) {
    const { users } = params;
    return await this.post<APIResponse & TestCampaignResponse>(
  ...
  async deleteUsers(user_ids: string[], options: DeleteUserOptions) {
    if (options?.user !== "soft" && options?.user !== "hard") {
      throw new Error(
        "Invalid delete user options. user must be one of [soft hard]",
      );
    }
    if (
      ...
      throw new Error(
        "Invalid delete user options. messages must be one of [soft hard]",
      );
    }
    if (
  ...
  async _createImport(
    path: string,
    options: CreateImportOptions = { mode: "upsert" },
  ) {
  ...
  async deletePushProvider({ type, name }: PushProviderID) {
    return await this.delete<APIResponse>(
      this.baseURL + `/push_providers/${type}/${name}`,
    );
  }
