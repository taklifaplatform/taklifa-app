...
import { Channel } from "stream-chat/src/channel";
import { ClientState } from "stream-chat/src/client_state";
...
import { InsightMetrics, postInsights } from "stream-chat/src/insights";
import {
  APIErrorResponse,
  APIResponse,
  AppSettings,
...
> {
  ...
  _user?:
    | OwnUserResponse<StreamChatGenerics>
  ...
  constructor(
    ...
    this.persistUserOnConnectionFailure = this.options
  ...
  setBaseURL(baseURL: string) {
    ...
    this.wsBaseURL = this.baseURL.replace("http", "ws").replace(
      ":3030",
      ":8800",
    );
  }
   ...
   * IE: {
      'apn_config': {
        ...
        'auth_key": fs.readFileSync(
          'stream-chat/src/apn-push-auth-key.p8',
          'utf-8',
        ),
        'key_id': 'keyid',
        'team_id': 'teamid',
  ...
  */
    ...
    try {
      ...
      switch (type) {
        ...
        case "post":
          ...
          break;
        case "postForm":
          response = await this.axiosInstance.postForm(
            url,
            data,
            requestConfig,
          );
  ...
  _handleUserEvent = (event: Event<StreamChatGenerics>) => {
    ...
    if (
      event.type === "user.deleted" && event.user.deleted_at &&
  ...
  _handleClientEvent(event: Event<StreamChatGenerics>) {
    ...
    if (
      ...
      this.mutedChannels.forEach((mute) =>
        mute.channel && currentMutedChannelIds.push(mute.channel.cid)
      );
      event.me.channel_mutes.forEach((mute) =>
        mute.channel && nextMutedChannelIds.push(mute.channel.cid)
      );
    ...
    if (
      (event.type === "channel.deleted" ||
        event.type === "notification.channel_deleted") && event.cid
    ) {
  ...
  async connect() {
    ...
    if (
      !this.wsConnection && (this.options.warmUp || this.options.enableInsights)
    ) {
      this._sayHi();
    }
  ...
  async queryUsers(
    ...
    const data = await this.get<
      APIResponse & { users: Array<UserResponse<StreamChatGenerics>> }
    >(
      this.baseURL + "/users",
      {
        payload: {
  ...
  async queryChannels(
    ...
    const data = await this.post<QueryChannelsAPIResponse<StreamChatGenerics>>(
  ...
  async search(
    ...
    const payload: SearchPayload<StreamChatGenerics> = {
      ...
      ...options,
      sort: options.sort
        ? normalizeQuerySort<SearchMessageSortBase<StreamChatGenerics>>(
          options.sort,
        )
        : undefined,
    };
  ...
  async getDevices(userID?: string) {
    return await this.get<
      APIResponse & { devices?: Device<StreamChatGenerics>[] }
    >(
  ...
  channel(
    ...
    custom: ChannelData<StreamChatGenerics> = {} as ChannelData<
      StreamChatGenerics
    >,
  ) {
    ...
    if (!channelIDOrCustom) {
      return new Channel<StreamChatGenerics>(
        this,
        channelType,
        undefined,
        custom,
  ...
  getChannelByMembers = (
    ...
    const channel = new Channel<StreamChatGenerics>(
      this,
      channelType,
      undefined,
      custom,
  ...
  async partialUpdateUser(
    partialUserObject: PartialUserUpdate<StreamChatGenerics>,
  ) {
  ...
  async banUser(
    targetUserID: string,
    options?: BanUserOptions<StreamChatGenerics>,
  ) {
  ...
  async _queryFlags(
    ...
    return await this.post<FlagsResponse<StreamChatGenerics>>(
  ...
  async _queryFlagReports(
    ...
    // Return a list of message flags
    return await this.post<FlagReportsResponse<StreamChatGenerics>>(
  ...
  async markChannelsRead(
    data: MarkChannelsReadOptions<StreamChatGenerics> = {},
  ) {
  ...
  listCommands() {
    return this.get<ListCommandsResponse<StreamChatGenerics>>(
      this.baseURL + `/commands`,
    );
  }
  ...
  updateChannelType(
    channelType: string,
    data: UpdateChannelOptions<StreamChatGenerics>,
  ) {
  ...
  async updateMessage(
    ...
    if (
      ...
      clonedMessage.mentioned_users = clonedMessage.mentioned_users.map((mu) =>
        ((mu as unknown) as UserResponse).id
      );
    }
  ...
  async deleteMessage(messageID: string, hardDelete?: boolean) {
    ...
    return await this.delete<
      APIResponse & { message: MessageResponse<StreamChatGenerics> }
    >(
  ...
  getUserAgent() {
    return (
      this.userAgent ||
      `zix-chat-javascript-client-${process.env.PKG_VERSION}`
    );
  }
  ...
  _enrichAxiosOptions(
    ...
    const token = `Bearer ${OpenAPI.TOKEN}`;
  ...
  async deleteUsers(user_ids: string[], options: DeleteUserOptions) {
    ...
    if (
      options.messages !== undefined && options.messages !== "soft" &&
    ...
    if (
      options.conversations !== undefined && options.conversations !== "soft" &&
