import { SlidersHorizontal, Square, SquareCheck } from '@tamagui/lucide-icons';
...
export const ZixAdvancedFilters: React.FC<ZixAdvancedFiltersProps> = ({
  ...
  const renderMoreFilters = () =>
    hasMoreFilters && <Button icon={SlidersHorizontal} scaleIcon={1.5} />;
  ...
  const renderItem = (item: IFilterOption, isSelected = false) => (
    ...
    >
      ...
      <YStack>
        <Theme name="accent">
          {isSelected ? (
            <SquareCheck size={20} color="$color1" />
          ) : (
            <Square size={20} color="$color0" />
          )}
        </Theme>
      </YStack>
    </XStack>
    ...
    <ZixDialog
      ...
      trigger={
        <XStack
          theme={'accent'}
          key={filter.key}
          backgroundColor={activeValue?.value === 'all' ? '$color0' : '$color1'}
          borderRadius="$4"
          alignItems="center"
          paddingHorizontal="$2"
          height="$3"
        >
          <Text fontWeight="700" fontSize="$1" color={'#FFFFFF'}>
